...
def download_credit_approval():
    """ Downloads `Credit Approval` from the UCI reporsitory
        unzipps it and saves it in the `data/external/credit-approval` directory 
    """
    logger = logging.getLogger(__name__)

    url_data = 'https://archive.ics.uci.edu/static/public/27/credit+approval.zip'
    # Send a GET request to the URL
    path_out = Path('data/external/credit-approval')
    path_out.mkdir(parents=True, exist_ok=True)
    path_zip = path_out / url_data.split('/')[-1]
    
    if path_zip.is_file():
        logger.info('file for Credit Approval already exists')
        return # Early exit
    
    logger.info('downloading Credit Approval')
...
def metadata_credit_approval():
    """ Extract metadata from doc file provided in the UCI documentation. 
        ...
        and then writes the output to data/processed/credit-approval/credit_approval_metadata.csv
    """
    logger = logging.getLogger(__name__)

    path_out_dir = Path('data/processed/credit-approval') 
    path_out_dir.mkdir(parents=True, exist_ok=True)
    path_out_csv = path_out_dir / 'credit_approval_metadata.csv'    
    if path_out_csv.is_file():
        logger.info('metadata file for Credit Approval already exists')
        return # Early exit
    logger.info('reading the content of metadata for Credit Approval')
    path_doc = Path('data/external/credit-approval/crx.names')
    ...
    with open(path_doc, 'r') as f:
        line = f.readline()
        while line:
            if within_found_attribute and line.strip().startswith('A'):
                attr_number_orig, attr_type_orig = line.split(':', 1)
                attr_number = int(attr_number_orig.strip()[1:])
                if 'continuous' in attr_type_orig:
                    attr_type = 'numerical'
                else:
                    attr_type = 'qualitative'
                attr_desc = ''
            ...
            elif line.startswith('8.  Missing Attribute Values'):
                break
            line = f.readline()
            
    logger.info('writting the content of metadata for Credit Approval to csv file')
...
def prepare_credit_approval():
    """ Prepare credit approval in csv file. 
    ...
    path_out_dir = Path('data/processed/credit-approval') 
    path_out_dir.mkdir(parents=True, exist_ok=True)
    path_out_csv = path_out_dir / 'credit_approval.csv'
    
    path_metadata_csv = Path('data/processed/credit-approval/credit_approval_metadata.csv')     
      
    path_inp_dir = Path('data/external/credit-approval') 
    path_inp_csv = path_inp_dir / 'crx.data'
    ...
    df = pd.read_csv(path_inp_csv, delimiter=',', header=None, names=lst_header, index_col=None, na_values=['?'])
    df = df [['target'] + lst_numerical + lst_qualitative]
    df['target'] = df['target'].map({'-':0, '+':1})
    df = pd.get_dummies(df, columns=lst_qualitative, drop_first=True, dtype=int)
    df.to_csv(path_out_csv, index=False)


def get_list_of_numerical_variables_credit_approval() -> list:
    """
...
def stratified_k_folds_credit_approval():
    """
    Prepares a file with the split into stratified 10-folds.
    Saves file credit_approval_folds.csv, where in each column there are observation
    numbers which will be used in test dataset in each fold. The observations
    for train subset are the remaining ones 
    """
    path_inp_dir = Path('data/processed/credit-approval') 
    path_inp_csv = path_inp_dir / 'credit_approval.csv'
    
    path_out_dir = Path('data/processed/credit-approval') 
    path_out_csv = path_out_dir / 'credit_approval_folds.csv'
    
    path_out_stats = path_out_dir / 'credit_approval_folds_stats.csv'
        
    df = pd.read_csv(path_inp_csv)
    # list of numerical variables:
    lst_numerical = get_list_of_numerical_variables_credit_approval()
...
def folds_to_csv_credit_approval():
    """
    ...
    # Read folds
    path_inp_folds = path_inp_dir / 'credit_approval_folds.csv'   
    df_folds = pd.read_csv(path_inp_folds)
    
    path_out_folds = Path('data/folds/credit-approval')
